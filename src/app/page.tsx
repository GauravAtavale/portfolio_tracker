...
export default function Home() {
  ...
  const [activeTab, setActiveTab] = useState<Record<number, "stats" | "news">>({});
  ...
  const fetchNews = useCallback(async (stockId: number, sym: string) => {
    if (newsCache[stockId]) return; // already fetched
    setNewsLoading((p) => ({ ...p, [stockId]: true }));
    try {
      const res = await fetch(`/api/news?symbol=${sym}`);
      const data = await res.json();
      setNewsCache((p) => ({ ...p, [stockId]: data.news ?? [] }));
    } finally {
      setNewsLoading((p) => ({ ...p, [stockId]: false }));
    }
  }, [newsCache]);
  ...
  function toggleRow(s: StockRow) {
    ...
    const isOpening = expandedId !== s.id;
    setExpandedId(isOpening ? s.id : null);
    if (isOpening) {
      // default to stats tab, fetch news in background
      setActiveTab((p) => ({ ...p, [s.id]: p[s.id] ?? "stats" }));
      fetchNews(s.id, s.symbol);
    }
  }

  function switchTab(stockId: number, tab: "stats" | "news") {
    setActiveTab((p) => ({ ...p, [stockId]: tab }));
  }
    ...
    <main>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: "1rem" }}>
        <div>
          ...
          <p style={{ color: "#888", marginBottom: "1rem" }}>Track your stock holdings and net worth in real time</p>
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: "0.75rem" }}>
          {lastUpdated && (
            <span style={{ color: "#888", fontSize: "0.85rem" }}>Last updated: {secondsAgo}s ago</span>
          )}
          <button className="btn-primary" onClick={fetchPortfolio} disabled={fetching}
      ...
      <form onSubmit={addStock}>
        ...
        <label>Quantity
          <input type="number" min="0.0001" step="any" value={quantity} onChange={(e) => setQuantity(e.target.value)} placeholder="10" required />
        </label>
        <label>Avg Cost ($)
          <input type="number" min="0.0001" step="any" value={avgCost} onChange={(e) => setAvgCost(e.target.value)} placeholder="150.00" required />
        </label>
      ...
      {portfolio && portfolio.stocks.length > 0 && (
        <table>
          <thead>
            <tr>
              <th>Symbol</th><th>Qty</th><th>Avg Cost</th><th>Live Price</th>
              <th>Market Value</th><th>Allocation</th><th>Cost Basis</th>
              ...
              <Fragment key={s.id}>
                ...
                <tr style={{ cursor: "pointer" }} onClick={() => toggleRow(s)}>
                  <td>
                    ...
                    {expandedId === s.id && <span style={{ color: "#555", fontSize: "0.75rem", marginLeft: "0.4rem" }}>▼</span>}
                  </td>
                  <td onClick={(e) => e.stopPropagation()}>
                    {editingId === s.id
                      ? <input type="number" step="any" value={editQty} onChange={(e) => setEditQty(e.target.value)} style={{ width: "80px" }} />
                      : s.quantity}
                  </td>
                  <td onClick={(e) => e.stopPropagation()}>
                    {editingId === s.id
                      ? <input type="number" step="any" value={editAvgCost} onChange={(e) => setEditAvgCost(e.target.value)} style={{ width: "90px" }} />
                      : fmt(s.avgCost)}
                  </td>
                  ...
                  <td onClick={(e) => e.stopPropagation()} style={{ whiteSpace: "nowrap" }}>
                    {editingId === s.id ? (
                      <>
                        <button className="btn-primary" style={{ padding: "0.2rem 0.6rem", fontSize: "0.8rem", marginRight: "0.4rem" }} onClick={() => saveEdit(s.id)}>Save</button>
                        <button className="btn-danger" onClick={() => setEditingId(null)}>Cancel</button>
                      </>
                    ) : (
                      <>
                        <button className="btn-edit" style={{ marginRight: "0.4rem" }} onClick={() => { setEditingId(s.id); setEditQty(String(s.quantity)); setEditAvgCost(String(s.avgCost)); setExpandedId(null); }}>✏️</button>
                    ...
                    <td colSpan={10} style={{ padding: 0 }}>
                      <div className="detail-card">
                        ...
                        <div className="tab-bar">
                          <button
                            className={`tab-btn ${(activeTab[s.id] ?? "stats") === "stats" ? "tab-active" : ""}`}
                            onClick={() => switchTab(s.id, "stats")}
                          >
                            📊 Stats
                          </button>
                          <button
                            className={`tab-btn ${activeTab[s.id] === "news" ? "tab-active" : ""}`}
                            onClick={() => switchTab(s.id, "news")}
                          >
                            📰 News
                          </button>
                        </div>
                        ...
                        {/* Stats Tab */}
                        {(activeTab[s.id] ?? "stats") === "stats" && (
                          <div className="detail-grid">
                            <div className="detail-item"><span className="detail-label">Day High</span><span className="detail-value">{fmt(s.regularMarketDayHigh)}</span></div>
                            <div className="detail-item"><span className="detail-label">Day Low</span><span className="detail-value">{fmt(s.regularMarketDayLow)}</span></div>
                            <div className="detail-item"><span className="detail-label">Prev Close</span><span className="detail-value">{fmt(s.regularMarketPreviousClose)}</span></div>
                            <div className="detail-item">
                              <span className="detail-label">Day Change</span>
                              <span className={`detail-value ${gainColor(s.regularMarketChangePercent)}`}>
                                {s.regularMarketChangePercent != null ? `${s.regularMarketChangePercent.toFixed(2)}%` : "—"}
                              </span>
                            </div>
                            <div className="detail-item"><span className="detail-label">52W High</span><span className="detail-value">{fmt(s.fiftyTwoWeekHigh)}</span></div>
                            <div className="detail-item"><span className="detail-label">52W Low</span><span className="detail-value">{fmt(s.fiftyTwoWeekLow)}</span></div>
                            ...
                            <div className="detail-item"><span className="detail-label">Holdings Value</span><span className="detail-value">{fmt(s.currentValue)}</span></div>
                          </div>
                        )}
                        ...
                        {activeTab[s.id] === "news" && (
                          <div className="news-list">
                            ...
                            {!newsLoading[s.id] && (newsCache[s.id] ?? []).length === 0 && (
                              <p className="news-empty">No news found for {s.symbol}.</p>
                            )}
